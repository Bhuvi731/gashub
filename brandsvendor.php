            ...
            <div class="container-fluid">
              <table id="example2" class="table table-bordered ">
                ...
                <tbody id="myTable">
                  ...
                  $vendor = pg_query($db, "SELECT petroleum.petroleum_name,petroleum.id,petroleum.vendorid,petroleum.status,petroleum.createdby,vendors.businessname from petroleum LEFT JOIN vendors ON vendors.id=petroleum.vendorid WHERE petroleum.status=1 AND petroleum.vendorid= $vendorid");
                  $i = 1;
...
<script>
  function save() {
    ...
    if (!/^[A-Za-z\s]+$/.test(brandname)) {
      alert("Brandname must contains only alphabets");
      return false;
    } else if (status == "") {
